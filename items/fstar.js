...
const EGACHA_COOLDOWN = 20 * 1000; // 20 seconds for fstar item
...
module.exports = {
  name: "fstar",
  description: "Guaranteed a Unique card",
  async execute(message, args) {
    ...
    if (!currencyData[userId]) {
      currencyData[userId] = {
        items: {
          fstar: 0, // Set default value for Gacha items
        },
      };
    }
    ...
    if (currencyData[userId].items.fstar === undefined) {
      currencyData[userId].items.fstar = 0; // Set default value for Gacha items
    }
    ...
    if (
      ...
      now - cooldownData[userId].gacha < currencyData[userId].items.fstar >= 1
    ) {
      // Check for fstar cooldown
      if (
        cooldownData[userId]?.egacha &&
        now - cooldownData[userId].egacha < EGACHA_COOLDOWN
      ) {
        // fstar item is still on cooldown
        const timeLeft = EGACHA_COOLDOWN - (now - cooldownData[userId].egacha);
      ...
      await message.reply(
        `You used an fstar item! You now have \`${currencyData[userId].items.fstar !== undefined
          ? currencyData[userId].items.fstar
          : "no"
        }\` fstar items left.`
      );
    }
    ...
    if (selectedImageAndFrame.rarity === "unique") {
      // Load the GIF from the fstar folder inside unique
      ...
      saveData(data);
      console.log(
        `\x1b[31m${message.author.username}\x1b[0m - \x1b[32m(${selectedImageAndFrame.characterName} || ${selectedImageAndFrame.animeName})\x1b[0m - \x1b[32m${printNumber}\x1b[0m - \x1b[32m${selectedImageAndFrame.rarity}\x1b[0m - \x1b[35m${message.guild.name}\x1b[0m`
      );
